...
#Grouping the data by race and party response, calculating the weighted total, and converting observations to variables (i.e. Response = Dem, Rep, Und) and the respective weighted total as observations
  ...
  spread(key = response, value = total, fill = 0) %>%   #Setting missing values to 0. Without this, when performing calculations later in the code (lines 87-90), NA values would make the final result NA. 
  ungroup() %>%
#Calculating the proportion of Dem, Rep, and Und responses relative to the total number of responses
  ...
  mutate(Rep = Rep/all) %>%
  mutate(Und = Und/all) %>%
  select(-all, -`3`) %>% #Selecting all variables except for "all" and the "3" responses 
  na_if(0) %>% #If a 0 is present (i.e. no data available), change it to NA. Later in the code (line 98), we replace missing data (NA) with a "-" to copy the original graph.
  gt() %>%
...
poll %>%
  ...
  geom_jitter(alpha = 0.5, size = 1) +    #Shifting the plotted data points slightly (with random variation) and decreasing the transparency to see their overlap
  theme_gray() + 
