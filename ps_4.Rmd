...
```{r include = FALSE}
#Filtering responses only for Democrats and Republicans and grouping by response
#Finding the lowest (i.e. earliest) timestamp for each group (Dem and Rep) 
#Converting observations (Dem and Rep) into columns (keys) and the lowest timestamp into the observations (values)
...
#Grouping the data by race and party response, calculating the weighted total, and converting observations to variables (i.e. Response = Dem, Rep, Und) and the respective weighted total as observations
  ...
  spread(key = response, value = total, fill = 0) %>% #Fill sets missing values to 0. Without this, when performing calculations later in the code (lines 87-90), NA values will make the final result NA. 
  ungroup() %>%
#Calculating the proportion of Dem, Rep, and Und responses relative to the total number of responses
  ...
  mutate(Rep = Rep/all) %>%
  mutate(Und = Und/all) %>%
  select(-all, -`3`) %>% #Select all variables except for "all" and the "3" responses 
  na_if(0) %>% #If a 0 is present (i.e. no data available), change it to NA. Later in the code (line 98), we replace missing data with a "-" to copy the original graph.
  gt() %>%
  ...
  fmt_percent(columns = vars(Dem, Rep, Und), decimals = 0) %>%   #Converting the graph's observations to percentages and rounding to 0 decimal places (whole numbers)
  fmt_missing(columns = 4, rows = 4, missing_text = "-") %>%   #Replacing missing text with "-"
...
```{r echo = FALSE, message = FALSE}
poll %>%
  filter(educ != "[DO NOT READ] Refused") %>%    #Filtering out "Do Not Read" data from the education variable
  mutate(educ = fct_relevel(educ, c("Grade school", 
                                    "High school", 
                                    "Some college or trade school",
                                    "Bachelors' degree", 
                                    "Graduate or Professional Degree"))) %>%   #Reorganizing the levels corresponding to education to match those on the original graph
  group_by(educ) %>%
  ggplot(aes(x = educ, y = final_weight)) + 
  coord_flip() +     #Flipping the x and y axes
  geom_violin() +    #Plotting a violin graph
  geom_jitter(alpha = 0.5, size = 1) +    #Shifting the plotted data points slightly (with random variation) and decreasing their transparency to see overlap
  theme_gray() + 
...
poll %>%
  select(response, gender, ager) %>%   #Selecting the relevant variables
  filter(ager != "[DO NOT READ] Refused", 
         response %in% c("Dem", "Rep", "Und")) %>%    #Filtering out "Do not read" observations from the age variable and "3" observations from the response variable
  group_by(gender, response, ager) %>%
  summarize(total = n()) %>%    #Calculating the total number of responses for each option (Rep, Dem, Und) within each combination of gender and age group
  ggplot(aes(x = response, y = total, fill = gender)) +
  geom_col(position = "dodge2") +   #Columns are placed side by side
  ...
  coord_flip() +    #Flipping the x and y axes for better visibility
  facet_wrap(~ager)     #Creating separate graphs of responses broken down gender for each of the age groups
```

#Colleagues
I collaborated with Alistair Gluck and Benjamin Hoffner-Brodsky. 